//~~
Sliver.ChanceOwner=Ext.extend(Ext.app.Module,{
    moduleType:"chance",
    moduleId:"chance-owner",
    init:function(){
        this.launcher={
            handler:this.createWindow,
            iconCls:"chance-owner-icon",
            scope:this,
            shortcutIconCls:"chance-owner-shortcut",
            text:SliverData.chance.title,
            tooltip:SliverData.chance.tooltip
        }
    },
    createWindow:function(){
		...
		if(!win){
            var winWidth=desktop.getWinWidth()/1.1;var winHeight=desktop.getWinHeight()/1.1;var store=new Ext.data.Store({
                proxy:new Ext.data.HttpProxy({
                    url:"chance/listChance.action"
                }),
                reader:new Ext.data.JsonReader({
                    root:"list",
                    totalProperty:"total",
                    id:"chanceID",
                    fields:["chanceID","chanceDate","chanceSource","chanceState","chancePercent","chanceTitle","chanceContent","chanceTotal","groupName"]
                }),
                remoteSort:true
            });store.setDefaultSort("chanceID","DESC");var expander=new Ext.grid.RowExpander({
			...
			cm.defaultSortable=true;var chanceFilterParams={
                "chance.chanceSource":"",
                "chance.chanceTitle":"",
			...
			function operateChanceParams(status,name){
                ...
                handler:function(){
                    ...
                    }store.baseParams=chanceFilterParams;store.setDefaultSort("chanceID","DESC");store.load({
                        params:{
                            start:0,
			...
			var clearButton=new Ext.Button({
                ...
                tooltip:"清空",
                handler:function(){
                    filterField.reset();store.baseParams={};store.setDefaultSort("chanceID","DESC");store.load({
                        params:{
                            start:0,
                            limit:pageSizePlugin.getPageSize()
                        }
			...
			var getChanceView=function(){
                var record=sm.getSelected();if(record){
                    var tabID=main.id+"-chance-"+record.data.chanceID;var temp=main.getComponent(tabID);if(!temp){
                        var temp=new Ext.Panel({
                            ...
                            temp.load({
                                ...
                                url:"chance/updateChanceView.action"
                            })
                        });var p=main.add(temp);main.setActiveTab(p)
                    }else{
                        temp.setTitle("销售机会["+record.data.chanceID+"]");temp.setIconClass("chance-owner-icon");temp.body.getUpdater().update({
                            scripts:true,
                            method:"post",
                            params:{
                                "chance.chanceID":record.data.chanceID,
			...
			var insertChanceView=function(){
                var tabID=main.id+"-chance-"+Ext.id();var temp=new Ext.Panel({
                    id:tabID,
                    iconCls:"chance-owner-icon",
                    autoScroll:true,
                    title:"新的销售机会",
                    ...
                    temp.load({
                        ...
                        url:"chance/insertChanceView.action"
                    })
                });var p=main.add(temp);main.setActiveTab(p)
            };
			...
			var removeChanceView=function(){
                var record=sm.getSelected();if(record){
                    Ext.MessageBox.confirm("删除销售机会","您确定要删除销售机会?",function(btn){
                        if(btn=="yes"){
                            Ext.Ajax.request({
                                url:"chance/removeChanceAction.action",
                                method:"post",
                                params:{
                                    "chance.chanceID":record.data.chanceID
                                },
                ...
                var record=sm.getSelected();if(record){
                    var tabID=Ext.id();var temp=new Ext.Panel({
                        ...
                        temp.load({
                            ...
                            url:"chance/insertOrderViewByChance.action"
                        })
                    });var p=main.add(temp);main.setActiveTab(p)
                }else{
                ...
                var record=sm.getSelected();if(record){
                    var tabID=Ext.id();var temp=new Ext.Panel({
                        ...
                        temp.load({
                            ...
                            url:"chance/insertOfferViewByChance.action"
                        })
                    });var p=main.add(temp);main.setActiveTab(p)
                }else{
                    Sliver.getDesktop().msg({
			...
			var getChanceStateView=function(){
                var tab=Ext.getCmp("chance-owner-bar");if(tab){
                    ...
                    var centerID=Ext.id();var center=new Ext.Panel({
                        ...
                        center.load({
                            ...
                            url:"chance/getChanceStateInformation.jsp"
                        })
			...
			var getChanceStatePieView=function(){
                var tab=Ext.getCmp("chance-owner-statepie");if(tab){
                    main.setActiveTab(tab)
                }else{
                    var centerID=Ext.id();var center=new Ext.Panel({
                        id:centerID,
                        ...
                        center.load({
                            ...
                            url:"chance/getChanceStatePieInformation.jsp"
                        })
                    });var west=new Ext.grid.PropertyGrid({
                        border:true,
                        region:"west",
			...
			win=desktop.createWindow({
                ...
                tbar:["-",{
                        ...
                        tooltip:"删除选中的销售机会",
                        iconCls:"chance-owner-icon-remove",
                        handler:removeChanceView
                    },"-",{
                        tooltip:"销售机会数量统计",
                        iconCls:"icon-chart-bar",
                        ...
                        handler:function(){
                            var tabID=Ext.id();var temp=new Ext.Panel({
                                ...
                                temp.load({
                                    ...
                                    url:"chance/insertCustomerView.action"
                                })
                            });var p=main.add(temp);main.setActiveTab(p)
                        }
                    },{
                        ...
                        handler:function(){
                            var tabID=Ext.id();var temp=new Ext.Panel({
                                ...
                                temp.load({
                                    ...
                                    url:"chance/insertContactView.action"
                                })
                            });var p=main.add(temp);main.setActiveTab(p)
                        }
                    }],
                items:main,
                taskbuttonTooltip:SliverData.chance.tooltip
            })
        }
		win.show()
    }
